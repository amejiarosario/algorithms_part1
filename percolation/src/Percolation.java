...
public class Percolation {
	 ...
	 * @param i row from 1 to N
	 * @param j column from 1 to N
	 * @exception java.lang.IndexOutOfBoundsException if either i or j is outside this range
	 */
	...
	public boolean isOpen(int i, int j) {
		i--; j--; // zero-indexed array conversion
		try{ checkBoundaries(i,j); } catch (IndexOutOfBoundsException e) { return false; }
		return open[i][j] == 1;
	}
	...
	private void checkBoundaries(int i, int j){
		if(i>=n || j>=n || i<0 || j<0) 
			throw new java.lang.IndexOutOfBoundsException("Either i or j is outside range N");
	}
}
